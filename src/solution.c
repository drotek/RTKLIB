...
extern int outubx(unsigned char *buff, const sol_t *sol, int *week, const ssat_t *ssat, const raw_t *raw)
{
    char *p=(char *)buff;
    ...
    static double dirp=0.0;
	signed long int posllh_lat, posllh_lon, posllh_height, diff_lat, diff_lon, diff_height; /* fusion edit */
	
	posllh_lat=raw->posllh_lat; /* fusion edit */
	posllh_lon=raw->posllh_lon; /* fusion edit */
	posllh_height=raw->posllh_height; /* fusion edit */
    int diff_flag=0; /* fusion edit -> flag = 0 : diff not valid, flag = 1 : diff valid */
	...
	/* fusion edit */	
	diff_lat=fabs(posllh_lat-lat);
	diff_lon=fabs(posllh_lon-lon);
	diff_height=fabs(posllh_height-hgt);
	...
	fprintf(f,"%ld\t%ld\t%ld\t%ld\t%ld\t%ld\t%ld\t%ld\t%ld\n", posllh_lat, posllh_lon, posllh_height,lat,lon,hgt,diff_lat,diff_lon,diff_height);
	fclose(f);		
	*/
	
	if(diff_lat>500 || diff_lon>500){
    ...
    soltocov(sol,P);
    h_acu=(unsigned long int)(((SQRT(P[4])+SQRT(P[0]))/2.0)*1000.0); /* mean of x & y standard deviation instead of horizontal accuracy */
    ...
    {
        *(chk_buff+16)=sol->ns<5?0x02:0x03; /* valid satellite # <5 given as 2D fix */
    }
    ...
    *(chk_buff+23)=0x00; /* no edop */
    ...
    {
        *(chk_buff+10)=sol->ns<5?0x02:0x03; /* valid satellite # <7 given as 2D fix */
    }
    
    if(sol->stat==SOLQ_FIX || sol->stat==SOLQ_FLOAT)
    {
        *(chk_buff+11)=0x8F;	/* RTK flag edit */
    }
    else
    {
...
extern int outsols(unsigned char *buff, const sol_t *sol, const double *rb,
                   const solopt_t *opt, const ssat_t *ssat, const raw_t *raw) /* ubx edit */ /* fusion edit */
{
    gtime_t time,ts={0};
    double gpst;
    int week,timeu;
    ...
    switch (opt->posf) {
        ...
        case SOLF_UBX:  p+=outubx(p,sol,&week,ssat,raw); break;   /* ubx edit */ /* fusion edit */
                   ...
                   const solopt_t *opt, const ssat_t *ssat, const raw_t *raw) /* ubx edit */ /* fusion edit */
{
    ...
    if ((n=outsols(buff,sol,rb,opt,ssat,raw))>0) {  /* ubx edit */ /* fusion edit */
        fwrite(buff,n,1,fp);
    }
}
/* output solution extended ----------------------------------------------------
