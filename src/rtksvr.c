...
static void writesol(rtksvr_t *svr, int index)
{
    ...
    for (i=0;i<2;i++) {
        /* output solution */
	n=outsols(buff,&svr->rtk.sol,svr->rtk.rb,svr->solopt+i,svr->rtk.ssat,&svr->raw[0]); /* ubx edit */ /* fusion edit */
    ...
    if (svr->moni) {
        n=outsols(buff,&svr->rtk.sol,svr->rtk.rb,&solopt,svr->rtk.ssat,&svr->raw[0]);    /* ubx edit */ /* fusion edit */
...
#endif
{
	...
	svr->raw[0].posllh_lat=0;
	svr->raw[0].posllh_lon=0;
	svr->raw[0].posllh_height=0;
	svr->raw[1].posllh_lat=0;
	svr->raw[1].posllh_lon=0;
	svr->raw[1].posllh_height=0;
	svr->raw[2].posllh_lat=0;
	svr->raw[2].posllh_lon=0;
	svr->raw[2].posllh_height=0;
